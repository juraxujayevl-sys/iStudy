...
import { Cpu, ExternalLink, Github, ArrowUpRight, Filter, Sparkles, Code2, Layers } from 'lucide-react';
import { AI_PROJECTS } from '../data/portfolioData';
...
export const AiEngineering: React.FC<AiEngineeringProps> = ({ onSelectProject }) => {
  ...
  return (
    <section id="ai-engineering" className="py-24 relative bg-[#090A0F] overflow-hidden border-t border-white/5">
      {/* Background glow */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full glass-panel border border-cyan-500/30 text-xs font-mono text-cyan-400 uppercase tracking-widest">
              ...
              <span>02 // AI Engineering</span>
            </div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight">
              Autonomous <span className="text-gradient-cyan">AI Systems & Agents</span>
            </h2>
            <p className="text-gray-400 text-sm sm:text-base">
              Production-ready multi-agent graphs, domain fine-tuned language models, semantic vector RAG search, and automated workflows.
            </p>
          </div>

          {/* Category Filter Pills */}
          <div className="flex flex-wrap items-center gap-2 bg-white/[0.03] p-1.5 rounded-2xl border border-white/10 max-w-full overflow-x-auto">
            {categories.map((cat) => (
              <button
                key={cat}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project) => (
            <div
              key={project.id}
              className="group rounded-3xl glass-panel border border-white/10 overflow-hidden hover:border-cyan-500/40 transition-all duration-300 flex flex-col justify-between hover:-translate-y-1.5 shadow-xl hover:shadow-cyan-500/10"
              id={`project-card-${project.id}`}
            >
              <div>
                {/* Cover Image Container */}
                <div className="relative aspect-[16/9] overflow-hidden bg-[#0F121A]">
                  <img
                    src={project.image}
                    alt={project.title}
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#151824] via-transparent to-transparent opacity-90" />
...
function featuredBadge(featured?: boolean) {
  ...
  return (
    <div className="absolute top-4 right-4">
      <span className="px-2.5 py-1 rounded-full bg-cyan-500 text-black text-[10px] font-mono font-bold flex items-center gap-1 shadow-lg shadow-cyan-500/30">
